...
def patch():
    ...
    while len(queue) > 0:
        ...
        csel_count = list_blocks[pc]['ins'].count('csel')
        if csel_count > 1:
            print "%x" % pc
            print 'not support multiple csel now.'
            return

        #find branch addr
        flag_is_branch = False
        cond = ''
        patch_addr = 0


        csel = list_blocks[pc]['capstone'][-2]
        b =   list_blocks[pc]['capstone'][-1]

        if b.mnemonic == 'ret':
            continue
        if b.mnemonic.startswith('cb'):
            continue

        if b.mnemonic[0] != 'b':
            print "real block not ends with b."
            return

        if csel.mnemonic == 'csel' and b.mnemonic == 'b':
            flag_is_branch = True
            cond = csel.op_str.split(', ')[-1]

        if csel_count > 0 and flag_is_branch == False:
            print "real block not ends with csel,b"
            return

        if flag_is_branch:
            patch_addr = csel.address
        else:
            patch_addr = b.address



        if len(flow[pc]) == 1:
            if flow[pc][0] != None:
                asm = asm_no_branch(patch_addr,flow[pc][0])
                patch_list.append((patch_addr,asm))
        else:

            asm = asm_has_branch(patch_addr,flow[pc][0],flow[pc][1],cond)
            patch_list.append((patch_addr, asm))
        for i in flow[pc]:
            if i != None:
                queue.append(i)
...
for i in md.disasm(bin1[offset:end],offset):
    ...
    if len(i.groups) > 0 or i.mnemonic == 'ret':
        ...
        list_blocks[block_item["saddress"]] = block_item
                #dot.node(hex(block_item["saddress"]),block_item['ins'] )
